...
class LayerGroupFilter:
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir, 'i18n', 'LayerGroupFilter_{}.qm'.format(locale)
        )
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginMenu(self.tr(u'&Layer Group Filter'), action)
    ...
    def run(self):
        ...
        if not self.pluginIsActive:
            ...
            if self.dockwidget == None:
                ...
                self.dockwidget.loadValuesButton.clicked.connect(
                    self.manualUpdateDistinctValues
                )

                self.dockwidget.presetComboBox.currentIndexChanged.connect(
                    self.presetChanged
                )
                self.dockwidget.layerGroupComboBox.currentIndexChanged.connect(
                    self.layerGroupChanged
                )
                self.dockwidget.filterValueComboBox.currentIndexChanged.connect(
                    self.updateGUI
                )
    ...
    def reloadProject(self):
        ...
        if QgsExpressionContextUtils.projectScope(QgsProject.instance()).variable(
            "groupFilterPresets"
        ):

            # pharse the XML string  to XML object and further initialize the plugin.
            self.filterXmlRoot = ET.fromstring(
            ...
            QMessageBox.warning(
                self.dockwidget,
                "No groupFilterPresets Set ",
                "No Filter Presets Set for this Project yet",
            )
    ...
    def loadFilterPresets(self):
        ...
        self.filterPresets = [
            ...
            for preset in self.filterXmlRoot.findall('preset')
        ]

        # Load in GUI
        self.dockwidget.presetComboBox.clear()
        self.dockwidget.presetComboBox.addItems(
            presetName[0] for presetName in self.filterPresets
        )
    ...
    def GetValue(self, presetXml):
        """check if a Filter Preset XML object has a Value List
        if:  no value Relation -> return = None
        else: return Value Relation XML object """
        vr = presetXml.findall("value")
        if not len(vr) == 0:
            return vr[0]  # only retrun first Value, others will be ignored
        else:
            return None

    def changeSubsetStrings(
        self, presetIndex=None, groupIndex=None, values=None, clearAll=False
    ):
        ...
        if not values:  # set default value from GUI as list with single value
            values = []
            fieldtext = self.dockwidget.filterValueComboBox.currentText()
            if fieldtext is not '':
                ...
                for filterColumn in filterColumns:
                    if filterColumn in fieldNames:
                        ...
                        for value in values:
                            ...
                            if firstValue:
                                addSubSetstring = "\"{0}\" {1} {2}{3}{4}".format(
                                    filterColumn, filterOper, opensep, value, closesep
                                )
                                firstValue = False
                            else:
                                addSubSetstring = (
    ...
    def getValueRelation(self, value, valueRelationXml, invert=False, getAll=False):
        ...
        exp = exp.replace(
            "\\", "\\\\"
        )  # work around in case a \ appears in value string
        ...
        try:
            if len(layers) > 1:  # set warning: layername not unique
                ...
                warningText = (
                    warningText
                    + "layer : {0} is not unique, only first instance is used \n".format(
                        vr["Layer"]
                    )
                )
            ...
            for feature in subset:
                ...
                if getAll:
                    ...
                    if resultCounter > 1:  # set warning: to many results
                        ...
                        warningText = (
                            warningText
                            + "More than one, matiching value found for : {0} , only first is used \n".format(
                                exp
                            )
                        )
            ...
            if resultCounter == 0:
                warning = True
                warningText = (
                    warningText + "No matiching value found for : {0} \n".format(exp)
                )
    ...
    def updateDistinctValues(self, findAll=False, manual=False):
        ...
        if not (
            autoUpdate == "TRUE" or manual
        ):  # autoUpdate: can be list of XML or String
        ...
        if findAll:
            ...
            groups = []
            groups.append(
                self.allGroups[self.dockwidget.layerGroupComboBox.currentIndex()]
            )
        ...
        for group in groups:
            for layer in group[1]:
                ...
                ):  # only vectorLayers have fields
                    for FilterColumn in FilterColumns:
                        if FilterColumn in [
                            field.name() for field in maplayer.fields()
                        ]:
        ...
        if (
            vr
        ):  # if has ValueRelation, we replace the found key's values with the values
            newUniqueValues = []
    ...
    def checkInputs(self, hasValueRelation=False):
        ...
        if self.dockwidget.presetComboBox.currentIndex() == -1:
            return False
        if self.dockwidget.layerGroupComboBox.currentIndex() == -1:
            return False
        if hasValueRelation:
            if self.dockwidget.filterValueComboBox.currentIndex() == -1:
                return False
        return True
    ...
    def loadXml(self):
        fileName = QFileDialog.getOpenFileName(
            self.dockwidget, "Select XML filter Preset XML ", "", '*.xml'
        )[0]
        xmlFile = open(fileName, 'r')
        presetXml = xmlFile.read().strip()
        xmlFile.close
        QgsExpressionContextUtils.setProjectVariable(
            QgsProject.instance(), 'groupFilterPresets', presetXml
        )
        self.reloadProject()
